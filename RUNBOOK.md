...
## STEP4-2  診療時間の採取（STEP4と同時に行われる）

診療時間はサイトを開いたその一度きりしか採れない。あとから欲しくなっても
97,000サイトの再巡回が必要になるため、STEP4が必ず同時に保存する。

保存先は `<出力プレフィクス>_verified_hours.jsonl`。この段階では解釈せず、
...
```bash
python3 pipeline/06_hours.py data/out1_verified_hours.jsonl > data/out1_hours.csv
```

出力列は 診療時間（整形） / 休診日 / 時間帯1 / 時間帯2 / 診療時間（原文）。
医療機関コードで取込用CSVと突き合わせて使う。

**この工程は巡回済みのデータを読むだけなので、何度でもやり直せる。**
解釈を変えたくなったら `06_hours.py` を直して再実行すればよく、再巡回は不要。
