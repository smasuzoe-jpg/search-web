...
| 時間帯1開始 / 時間帯1終了 | 開始と終了を別セルに分ける |
| 時間帯2開始 / 時間帯2終了 | 2診制のとき |
| 時間帯3開始 / 時間帯3終了 | 3診制のとき |
| 診療科目 | 標榜科を「・」でつないだもの |
| 診療科目1〜8 | 標榜科を1つずつ別セル |
| 診療科目数 | 検出した標榜科の数 |
| 診療科目の出所 | 見出し / 診療科目欄 / メニュー / 施設名 |
| 基本領域 | 基本領域19診療科へ畳んだもの |
| 基本領域1〜5 | 基本領域を1つずつ別セル |
| 基本領域数 | 該当した基本領域の数 |
| 診療時間の取得元 | どのページから採ったか |
| 診療時間（原文） | 採取した生テキスト |

時間帯の開始と終了を分けてあるので、「18時以降も診療している医院」のような
絞り込みが表計算ソフトでそのままできる。

### 休診日の正規化

サイトの文章には「1月5日(月)から通常通り診療」「その他に臨時休診あり」のような
臨時のお知らせが混ざる。`pipeline/closed.py` で次のものだけを残す。
...
日付に付く曜日（`1月5日(月)`）は休診日ではないので、先に取り除いてから判定する。
これをしないと臨時休診のお知らせから曜日を拾ってしまう。

診療科目は出所をまたいで統合する。見出しだけでは標榜科の一部しか載っていない
ことが多く、診療科目欄のほうが網羅的なため。

### 診療科目の2軸

細かい標榜科と、粗い基本領域19診療科の両方を出す。集計は基本領域、
個別の見極めは標榜科、と使い分けられる。

**基本領域19診療科（この順で並ぶ）**

内科、小児科、皮膚科、精神科、外科、整形外科、産婦人科、眼科、耳鼻咽喉科、
泌尿器科、脳神経外科、放射線科、麻酔科、病理、臨床検査、救急科、形成外科、
リハビリテーション科、総合診療科

**区切り文字は軸ごとに変えてある。** 取り違えを防ぐため。

| 列 | 区切り | 例 |
|---|---|---|
| 診療科目 | `・` | `循環器内科・糖尿病内科・内科・アレルギー科` |
| 基本領域 | `;` | `耳鼻咽喉科;小児科` |

例: 循環器内科・糖尿病内科・内科・アレルギー科・呼吸器内科・禁煙外来・予防接種
→ 基本領域は「内科」1つに畳まれる。

割り当て表は `pipeline/depts.py` の `TO_BASIC`。**書き換えれば割り当てを変えられ、
再巡回は不要。** 判断が分かれるものは以下のとおり。

| 標榜科 | 割り当て先 | 理由 |
|---|---|---|
| 心療内科 | 精神科 | 本来は内科系だが実務上は精神科との併設が多い |
| 脳神経内科・神経内科 | 内科 | 内科の一分野 |
| リウマチ科・アレルギー科 | 内科 | 小児側の場合もあるが内科に寄せた |
| 美容外科 | 形成外科 | |
| 美容皮膚科 | 皮膚科 | |
| 訪問診療・在宅診療 | 総合診療 | |
| 歯科系・健診・予防接種など | 対象外 | 19領域は医科のみ |
