...
## STEP4-2  診療時間と診療科目の採取（STEP4と同時に行われる）

診療時間も診療科目も、サイトを開いたその一度きりしか採れない。あとから欲しく
なっても97,000サイトの再巡回が必要になるため、STEP4が必ず同時に保存する。

保存先は `<出力プレフィクス>_verified_details.jsonl`。この段階では解釈せず、
診療時間らしい塊を生テキストのまま回収する。
...
トップページで見つからない場合、診療案内ページを1枚だけ追加で開く。
...
```bash
python3 pipeline/06_details.py data/out1_verified_details.jsonl > data/out1_details.csv
```

出力列は 診療時間（整形） / 休診日 / 時間帯1 / 時間帯2 / 診療科目 /
診療科目の出所 / 診療時間（原文）。

診療科目は「呼吸器内科」を「内科」と取り違えないよう長い科名から当てる。
出所は 見出し → 診療科目欄 → メニュー → 施設名 の順に信頼度が高い。
**サイトから取れない場合も施設名から拾うので、巡回できなかった施設でも埋まる。**
医療機関コードで取込用CSVと突き合わせて使う。

**この工程は巡回済みのデータを読むだけなので、何度でもやり直せる。**
解釈を変えたくなったら `06_details.py` を直して再実行すればよく、再巡回は不要。

診療時間を画像で載せているサイトからは取得できない。日本の医院サイトでは
珍しくないので、取得率は100%にはならない。
